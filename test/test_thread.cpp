...
int main(int argc, const char **argv)
{
    ...
    lutrpc::ThreadGroup thread_group(2);
    lutrpc::Thread *thread = thread_group.getThread();
    thread->getReactor()->addEvent(event);
    thread->getReactor()->addTimer(pTimer);

    lutrpc::Thread *thread2 = thread_group.getThread();
    thread2->getReactor()->addTimer(pTimer);
